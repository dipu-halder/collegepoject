...
const Categories = () => {
  ...
  const handleAddToCart = (item) => {
    // normalize stored cart item keys
    const newItem = {
      title: item.name,
      description: item.description,
      // prefer absolute if provided, otherwise keep relative path (backend will normalize too)
      image: item.img && (item.img.startsWith("http") ? item.img : item.img),
      price: Number(item.pices),
      quantity: 1,
      tag: item.type
    };
  ...
  return (
    <div className="categories-page">
      ...
      <div className="filters">
        ...
        <label onClick={() => handleFilter("low-high")}>Low → High</label>
        <label onClick={() => handleFilter("high-low")}>High → Low</label>
        <label onClick={() => handleFilter("clear")} style={{ color: "red" }}>Clear</label>
      </div>

      <p style={{ fontSize: "18px", fontWeight: "bold", marginBottom: 10 }}>{filterTitle}</p>
      ...
      <div>
        <Link to="/cart"><button className="cart-float-btn"><IonIcon icon={cartOutline} /></button></Link>
      </div>
    </div>
  );
};
