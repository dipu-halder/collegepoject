...
//       });
//       setOrder(res.data);
//       if (res.data.riderLocation) setRiderLocation(res.data.riderLocation);
//     } catch (err) {
...
//         <p><span className="font-semibold">ETA:</span> Coming soon ⏳</p> 
...
export default function TrackOrder() {
  ...
  // authorizationToken from context is usually "Bearer <token>" or null
  // OrderTrackingMap accepts either "Bearer <token>" or raw token (createSocket normalizes)
  const token = authorizationToken || localStorage.getItem("token") || null;
  ...
  return (
    <div style={{ padding: 12 }}>
      ...
      <OrderTrackingMap orderId={orderId} token={token} />
    </div>
  );
}
