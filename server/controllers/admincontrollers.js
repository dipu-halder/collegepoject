...
//     const id = req.params.id;
//     const updatedUserData = req.body;
//     const updateUser = await User.findByIdAndUpdate(id, updatedUserData, { new: true }).select("-password");
//     return res.status(200).json(updateUser);
//   } catch (error) {
//     next(error);
//   }
...
//   try {
//     // Populate both user and assignedRider so frontend can show rider details
//     const orders = await Order.find()
//       .sort({ createdAt: -1 })
//       .populate("user", "username email phone")
//       .populate("assignedRider", "name phone vehicle isApproved");
...
//         isApproved: order.assignedRider.isApproved
//       } : null,
...
//   try {
//     const { riderId } = req.body; // frontend se riderId bheja jayega
//     const orderId = req.params.id;

//     // Check order exist karta hai ya nahi
//     const order = await Order.findById(orderId);
//     if (!order) {
//       return res.status(404).json({ message: "Order not found" });
//     }

//     // Check rider exist karta hai aur approve hai ya nahi
//     const rider = await Rider.findById(riderId);
//     if (!rider) {
//       return res.status(404).json({ message: "Rider not found" });
//     }
//     if (!rider.isApproved) {
//       return res.status(400).json({ message: "Rider is not approved yet" });
//     }

//     // Assign rider
//     order.assignedRider = riderId;
//     order.status = "Assigned"; // optional: tum chaho to Pending → Assigned kar sakte ho
//     await order.save();
